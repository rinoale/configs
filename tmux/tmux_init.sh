...
tjs() {
  result=$(session_check $tmux_name_tjs)
  echo "session status : ${result}"
  if [[ ${result} == 0 ]]; then
    echo "${tmux_name_tjs} session already attached"
  else
    echo "${tmux_name_tjs} session not attached yet"
    echo "Starting ${tmux_name_tjs}"
    init_comm=$(init_command_tjs)
    eval ${init_comm}
  fi
}
...
init_command_tjs() {
  new_session="tmux new -d -s ${tmux_name_tjs} \;"
  attach=" attach\;"
  split_mask2=" split-window -h\;"
  split_mask3=" split-window -v\;"
  select_mask0_window=" select-pane -t 0\;"
  resize0=" resize-pane -t 0 -y 70\;"
  resize1=" resize-pane -t 1 -y 70\;"

  open_mask1_vim=" send-keys -t 0 'cd ~/git/tapjoyserver && vim .' Enter \;"
  open_mask2_vim=" send-keys -t 1 'cd ~/git/tapjoyserver' Enter\;"
  open_mask3_vim=" send-keys -t 2 'cd ~/git/tapjoyserver' Enter\;"
  echo $new_session$attach$split_mask2$split_mask3$resize1$select_mask0_window$split_mask3$resize0$open_mask1_vim$open_mask2_vim$open_mask3_vim
}
