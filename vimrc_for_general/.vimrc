...
function! s:ShowCheat()
  ...
  call setline(1, [
    ...
    \ '- `[^,]*` — match until comma; `[^x]` excludes one character, not a string',
    \ '- `\(text\)\@!` — negative lookahead: next text must not match `text`',
    \ '- `:%s/object_a\.\(method_a,\)\@![^,]\+,/object_a.method_b,/g` — replace methods except `method_a,`',
    \ '- `:%s/^\(\s*\)def \(\k\+\)\>.*$/&\r\1  @\2 ||=/` — add Ruby memoization line after each method',
    \ '- `:%s/\(.*\) (\(\d\+\))/\2 \1/g` — reorder with capture groups: `Alice (25)` → `25 Alice`',
    \ '- `\v` (very magic) — unescaped `()` for groups: `:%s/\v(.+) \((\d+)\)/\2 \1/g`',
    \ '',
    \ '## Selection',
